...
def upgrade() -> None:
    ...
    op.create_table('call_extracted_data',
        ...
        sa.ForeignKeyConstraint(['call_id'], ['audio_calls.call_id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    
    # Create index on call_id for better query performance
    op.create_index(op.f('ix_call_extracted_data_call_id'), 'call_extracted_data', ['call_id'], unique=False)
    op.create_index(op.f('ix_call_extracted_data_id'), 'call_extracted_data', ['id'], unique=False)


def downgrade() -> None:
    # Drop indexes
    op.drop_index(op.f('ix_call_extracted_data_id'), table_name='call_extracted_data')
    op.drop_index(op.f('ix_call_extracted_data_call_id'), table_name='call_extracted_data')
